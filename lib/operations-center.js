...
function buildRecentOperations(remote, deliveries, activities) {
  const jobs = (remote.jobs || []).map((job) => ({
    ...
    detail: job.error || job.progress?.message || "",
...
function transient(error) {
  return /timeout|timed out|fetch failed|ECONN|ENOTFOUND|HTTP 429|HTTP 5\d\d/i.test(error?.message || String(error));
}
...
async function checkOneService(serviceName, { required = false, remote = null } = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      lastError = error;
      if (attempt === 2 || !transient(error)) break;
