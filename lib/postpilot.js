...
async function fetchVideoPostDetails(videoId, pageAccessToken) {
  if (!videoId) return {};
  const fieldSets = [
    "id,post_id,permalink_url,status,created_time",
...
async function publishToFacebook({ file, caption, firstComment }) {
  ...
  const videoDetails = mediaType === "video"
    ...
    : {};
  const postId = mediaResponse.post_id || videoDetails.post_id || mediaResponse.id || "";
  ...
  const processingNote = mediaType === "video" && !videoDetails.permalink_url
