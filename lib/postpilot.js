...
function buildStory({ hook, productName, promise, productContext, creativeType, angle }) {
  ...
  if (lower.includes("tanpa produk sendiri") || lower.includes("produk digital")) {
    return `${hook} Biasanya orang bukan malas, cuma terlalu lama duduk dalam fasa tunggu \"nanti bila dah ada idea produk sendiri\". Hari bertukar minggu, minggu bertukar bulan, tapi masih ulang benda yang sama: simpan niat, tengok orang lain jalan dulu, kemudian rasa tertinggal. Salespage ${productName} masuk tepat pada titik itu dengan mesej utama: ${promise}${angleSentence}`;
  }

  if (lower.includes("ads") || lower.includes("funnel") || lower.includes("whatsapp")) {
    return `${hook} Ramai orang nampak masalah di permukaan sahaja: iklan mahal, leads senyap, prospek hilang, atau sales tak naik. Tapi bila diteliti, selalunya ada satu proses yang tidak kemas di belakangnya. Salespage ${productName} bawa cerita ini dengan mesej utama: ${promise}${angleSentence}`;
  }

  if (lower.includes("course") || lower.includes("kelas") || lower.includes("belajar")) {
    return `${hook} Kadang-kadang yang membuatkan orang lambat bergerak bukan sebab tiada minat, tapi sebab terlalu banyak maklumat bercampur sampai tidak tahu nak mula di mana. Salespage ${productName} cuba susun semula kekusutan itu dengan mesej utama: ${promise}${angleSentence}`;
  }

  return `${hook} Selalunya orang mula ambil serius bila masalah yang sama berulang beberapa kali. Mula-mula nampak kecil, kemudian ia ganggu masa, fokus, dan keputusan. Salespage ${productName} membawa cerita itu kepada satu mesej utama: ${promise}${angleSentence}`;
}
