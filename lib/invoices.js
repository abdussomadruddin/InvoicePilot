...
async function buildClientWhatsappReminder(body) {
  ...
  if (type === "custom") {
    ...
    throw new Error("Pilih WhatsApp Invoice, Receipt, atau Custom Message.");
  }
...
function reportMoney(value) {
  ...
  if (Number.isInteger(number)) return `RM${number.toLocaleString("en-MY")}`;
...
function normalizeReportInput(body = {}, client) {
  ...
  const leadsGenerated = reportNumber(body.leadsGenerated, "Leads generated", { integer: true });
  const costPerLead = Object.prototype.hasOwnProperty.call(body, "costPerLead") && String(body.costPerLead).trim() !== ""
    ? reportNumber(body.costPerLead, "Cost per lead")
    : (leadsGenerated ? Math.round((adSpend / leadsGenerated) * 100) / 100 : 0);
  const targetLeads = reportNumber(body.targetLeads, "Target leads", { integer: true });
  const targetCpl = reportNumber(body.targetCpl, "Target CPL");
  ...
  return {
    ...
    phase: String(body.phase || "SETUP PHASE").trim().toUpperCase(),
    reportTitle: String(body.reportTitle || "EXECUTIVE LEAD GENERATION BRIEF").trim().toUpperCase(),
    adSpend,
    leadsGenerated,
    costPerLead,
    targetLeads,
...
function writeReportWrapped(doc, text, x, y, width, options = {}) {
  doc
    ...
    .fillColor(options.color || "#14213d")
...
function renderReportPdf(report) {
  return new Promise((resolve, reject) => {
    ...
    const metrics = [
      ["AD SPEND", reportMoney(report.adSpend), blue],
      ["LEADS GENERATED", String(report.leadsGenerated), green],
      ["COST PER LEAD", reportMoney(report.costPerLead), red],
    ];
    ...
    proofLines.slice(0, 6).forEach((line, index) => {
      const y = 266 + index * 15;
      doc.circle(leftX + 16, y + 4, 2.6).fill(green);
      writeReportWrapped(doc, line, leftX + 25, y, colW - 37, { size: 8.5 });
    });

    reportSection(doc, "BEST PERFORMER", rightX, 238, colW, 127);
    writeReportWrapped(doc, `Winning Creative: ${report.winningCreative}`, rightX + 12, 268, colW - 24, { bold: true, size: 8.5 });
    writeReportWrapped(doc, `Performance: ${report.bestPerformance || `${report.leadsGenerated} Leads | ${reportMoney(report.costPerLead)} CPL`}`, rightX + 12, 288, colW - 24, { size: 8.5 });
    writeReportWrapped(doc, `Audience: ${report.bestAudience}`, rightX + 12, 308, colW - 24, { size: 8.5 });

    reportSection(doc, "LEAD LEAKS", leftX, 382, colW, 102);
    const leaks = report.leadLeaks.length ? report.leadLeaks : ["Tiada lead leak kritikal direkodkan untuk minggu ini."];
    let leakY = 410;
    leaks.slice(0, 4).forEach((line) => {
      writeReportWrapped(doc, line, leftX + 12, leakY, colW - 24, { size: 8.4 });
      leakY += doc.heightOfString(line, { width: colW - 24, lineGap: 1.5 }) + 7;
    });
    ...
    doc.font("Helvetica-Bold").fontSize(8.5).fillColor(red).text("NEXT TARGET", rightX + colW / 2 + 4, 398, { width: colW / 2 - 20, align: "center" });
    doc.moveTo(rightX + colW / 2, 392).lineTo(rightX + colW / 2, 474).strokeColor("#e8f0ff").lineWidth(2).stroke();
    doc.font("Helvetica-Bold").fontSize(19).fillColor(ink).text(`${report.leadsGenerated} Leads`, rightX + 12, 424, { width: colW / 2 - 20, align: "center" });
    doc.fontSize(12).fillColor(muted).text(`${reportMoney(report.costPerLead)} CPL`, rightX + 12, 449, { width: colW / 2 - 20, align: "center" });
    doc.font("Helvetica-Bold").fontSize(19).fillColor(ink).text(`${report.targetLeads} Leads`, rightX + colW / 2 + 4, 424, { width: colW / 2 - 20, align: "center" });
    doc.fontSize(12).fillColor(muted).text(`${reportMoney(report.targetCpl)} CPL`, rightX + colW / 2 + 4, 449, { width: colW / 2 - 20, align: "center" });

    reportSection(doc, "NEXT 7 DAYS", margin, 503, contentW, 125);
    const nextItems = report.next7Days.length ? report.next7Days : ["Monitor campaign performance.", "Review CPL and lead quality.", "Prepare next optimization action."];
    nextItems.slice(0, 6).forEach((line, index) => {
      const rowY = 532 + index * 14;
      ...
      writeReportWrapped(doc, line, margin + 38, rowY - 1, contentW - 54, { size: 8.4 });
    });

    reportSection(doc, "EXECUTIVE RECOMMENDATION", margin, 648, contentW, 100, { fill: "#ffffff" });
    writeReportWrapped(doc, report.recommendation || "Recommendation belum diisi.", margin + 12, 678, contentW - 24, { size: 9, lineGap: 2 });
